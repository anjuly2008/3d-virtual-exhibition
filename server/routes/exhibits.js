...
router.get('/likes/mine', authMiddleware, (req, res) => {
  try {
    const exhibits = db.exhibits.findLikedByUserId(req.user.id);
...
router.get('/mine', authMiddleware, (req, res) => {
  try {
    const exhibits = db.exhibits.findByCreatorId(req.user.id);
...
router.get('/', (req, res) => {
  try {
    const { category, search, tags, usage } = req.query;

    const parsedTags = tags ? JSON.parse(tags) : [];
    const parsedUsage = usage ? JSON.parse(usage) : [];
...
router.post(
  ...
  (req, res) => {
    try {
      ...
      const modelUrl = req.files && req.files.model
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      const liked = db.exhibits.hasLiked(
        userId,
        exhibitId
      );

      const exhibit = db.exhibits.findById(
        exhibitId
      );
